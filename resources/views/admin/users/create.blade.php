    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('avatar') ? 'is-invalid' : '' }}" name="avatar" id="avatar">{{ old('avatar') }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('chat_rooms') ? 'is-invalid' : '' }}" name="chat_rooms[]" id="chat_rooms" multiple>
                    @foreach($chat_rooms as $id => $chat_room)
                        <option value="{{ $id }}" {{ in_array($id, old('chat_rooms', [])) ? 'selected' : '' }}>{{ $chat_room }}</option>
                    @endforeach
                </select>
