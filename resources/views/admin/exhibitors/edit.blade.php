...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.exhibitors.update", [$exhibitor->slug]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('admins') ? 'is-invalid' : '' }}" name="admins[]" id="admins" multiple required>
                    @foreach($admins as $id => $admin)
                        <option value="{{ $id }}" {{ (in_array($id, old('admins', [])) || $exhibitor->admins->contains($id)) ? 'selected' : '' }}>{{ $admin }}</option>
                    @endforeach
                </select>
