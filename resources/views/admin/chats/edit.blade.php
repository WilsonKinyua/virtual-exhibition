    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.chats.update", [$chat->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('chat_room') ? 'is-invalid' : '' }}" name="chat_room_id" id="chat_room_id" required>
                    @foreach($chat_rooms as $id => $entry)
                        <option value="{{ $id }}" {{ (old('chat_room_id') ? old('chat_room_id') : $chat->chat_room->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('message_type') ? 'is-invalid' : '' }}" type="text" name="message_type" id="message_type" value="{{ old('message_type', $chat->message_type) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" type="text" name="status" id="status" value="{{ old('status', $chat->status) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('attachment') ? 'is-invalid' : '' }}" name="attachment" id="attachment">{{ old('attachment', $chat->attachment) }}</textarea>
